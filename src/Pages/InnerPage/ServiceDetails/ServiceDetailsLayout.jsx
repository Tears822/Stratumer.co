...
import { serviceCategories } from "./serviceDetailsData";

const ServiceDetailsLayout = ({ content, activePath }) => {
  return (
    <>
      <BreadCrumb
        breadCrumbTitle={content.title}
        ...
        <div className="Container">
          <div className="grid grid-cols-3 gap-[50px]">
            <div className="col-span-3 lg:col-span-2">
              ...
              <p className="font-Nunito text-TextColor2-0 mt-6">{content.intro}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-7">
                ...
                <div className="flex flex-col md:flex-row lg:flex-col xl:flex-row gap-5 bg-[#f3f4f8] rounded-md px-6 py-7">
                  ...
                  <div className="flex-1">
                    <h6 className="font-Rajdhani font-semibold text-[22px] text-HeadingColor-0">
                      {content.featureTwoTitle}
                    </h6>
                    <p className="font-Nunito text-TextColor2-0 mt-3">
                      {content.featureTwoDesc}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-7 mt-9">
                <div className="text-center group">
                  ...
                  <div className="bg-white shadow-cases rounded inline-block pt-7 pl-4 sm:pl-8 pb-7 w-4/5 -mt-5 relative z-10">
                    <h4 className="font-Rajdhani text-HeadingColor-0 font-semibold text-[22px] lg:text-xl xl:text-[22px] text-left mb-3">
                      {content.benefitOneTitle}
                    </h4>
                    <ul>
                      {content.benefitOneItems.map((item) => (
                        <li
                          key={item}
                          className="flex items-center gap-2 text-TextColor2-0 text-left font-Nunito mb-2 lg:text-sm xl:text-base"
                        >
                ...
                <div className="text-center group">
                  ...
                  <div className="bg-white shadow-cases rounded inline-block pt-7 pl-4 sm:pl-8 pb-7 w-4/5 -mt-5 relative z-10">
                    <h4 className="font-Rajdhani text-HeadingColor-0 font-semibold text-[22px] lg:text-xl xl:text-[22px] text-left mb-3">
                      {content.benefitTwoTitle}
                    </h4>
                    <ul>
                      {content.benefitTwoItems.map((item) => (
                        <li
                          key={item}
                          className="flex items-center gap-2 text-TextColor2-0 text-left font-Nunito mb-2 lg:text-sm xl:text-base"
                        >
            ...
            <div className="col-span-3 lg:col-span-1">
              <div className="bg-[#f3f4f8] rounded-md px-4 sm:px-8 lg:px-6 xl:px-8 pt-7 pb-4 mb-7">
                ...
                <ul className="mt-8">
                  {serviceCategories.map(({ label, path }) => {
                    const isActive = path === activePath;
                    return (
                      <li key={path}>
                        <Link to={path}>
                          <button
                            className={`w-full font-Nunito text-left transition-all duration-500 group px-7 py-4 flex items-center justify-between rounded mb-3 overflow-hidden z-[1] relative before:absolute before:top-0 before:right-0 before:w-0 before:-z-[1] before:h-full before:bg-PrimaryColor-0 before:rounded before:transition-all before:duration-500 hover:before:w-full hover:before:left-0 hover:text-white ${
                              isActive
                                ? "text-white bg-PrimaryColor-0"
                            ...
                            <span className="flex items-center gap-3 lg:gap-1 xl:gap-3">
                              <FaRegFolderOpen
                                className={`transition-all duration-500 group-hover:text-white ${
                                  isActive ? "text-white" : "text-PrimaryColor-0"
                                }`}
                              />
                              {label}
                            </span>
